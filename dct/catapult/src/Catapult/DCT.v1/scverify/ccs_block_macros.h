...
#ifdef  CCS_DESIGN_FUNC_DCT
extern void mc_testbench_capture_IN( ac_channel<ac_fixed<20, 8, true, AC_TRN, AC_WRAP > > &dst,  ac_channel<ac_fixed<20, 8, true, AC_TRN, AC_WRAP > > &src);
extern void mc_testbench_capture_OUT( ac_channel<ac_fixed<20, 8, true, AC_TRN, AC_WRAP > > &dst,  ac_channel<ac_fixed<20, 8, true, AC_TRN, AC_WRAP > > &src);
extern void mc_testbench_wait_for_idle_sync();

#define ccs_intercept_DCT_26 \
  ccs_real_DCT(ac_channel<ac_fixed<20, 8, true, AC_TRN, AC_WRAP > > &dst,ac_channel<ac_fixed<20, 8, true, AC_TRN, AC_WRAP > > &src);\
  void DCT(ac_channel<ac_fixed<20, 8, true, AC_TRN, AC_WRAP > > &dst,ac_channel<ac_fixed<20, 8, true, AC_TRN, AC_WRAP > > &src)\
{\
    static bool ccs_intercept_flag = false;\
    if (!ccs_intercept_flag) {\
      std::cout << "SCVerify intercepting C++ function 'DCT' for RTL block 'DCT'" << std::endl;\
      ccs_intercept_flag=true;\
    }\
    mc_testbench_capture_IN(dst,src);\
    ccs_real_DCT(dst,src);\
    mc_testbench_capture_OUT(dst,src);\
}\
  void ccs_real_DCT
#else
#define ccs_intercept_DCT_26 DCT
#endif //CCS_DESIGN_FUNC_DCT
...
// conv_from_fraction 1375 INLINE
#define ccs_intercept_conv_from_fraction_1375 conv_from_fraction
// set_slc 1510 INLINE
...
// ldexpr 222 INLINE
#define ccs_intercept_ldexpr_222 ldexpr
// iv_mult 438 INLINE
