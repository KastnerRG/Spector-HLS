...
#pragma design top
void DCT(ac_channel<fl1> &dst,ac_channel<fl1> &src){
DATA_MEM mdst, msrc;
#ifndef __SYNTHESIS__
	while(dst.available(size_im))
#endif
	{
	
		loop_lmm:for(unsigned i=0; i<size_im; i++)
			msrc.data[i] = src.read();
					...
					loop_4:for(int l=0;l<blockdim_x;l++){
						loop_5:for(int x = 0; x < 8; x++){
							...
							loop_6:for(int y = 0; y< 8; y++)
								buf[x] += msrc.data[ ((i+l) + k) * stride + (j)+y] * C[(2*y*x+x)%16]*(((2*y*x+x)/16)%2?-1:1);
						}
						mdst.data[ (i + k+l) * stride + (j)]=(buf[0]*C_norm);
						loop_7:for(int z = 1; z < 8; z++)
							mdst.data[ (i + k+l) * stride + (j)+z]=(buf[z] * fl1(0.5));
					...
					loop_10:for(int m=0;m<blockdim_y;m++){
						loop_11:for(int x = 0; x < 8; x++){
							...
							loop_12:for(int y = 0; y< 8; y++)
								buf[x] +=  mdst.data[ (i) * stride + ((j+m) + k)+(y*stride)] * C[(2*y*x+x)%16]*(((2*y*x+x)/16)%2?-1:1);
						}
						mdst.data[(i) * stride + ((j+m) + k)]=(buf[0]* C_norm);
						loop_13:for(int z = 1; z < 8; z++)
							mdst.data[(i) * stride + ((j+m) + k)+(z*stride)]=( buf[z] * fl1(0.5));
	            //DCT8_auto(dst [(i) * stride + ((j+m) + k)], dst [ (i) * stride + ((j+m) + k)], stride, stride);
					}
				}
			}
		}
		loop_out:for(unsigned i=0; i<size_im; i++)
			dst.write(mdst.data[i]);
	}
}
