...
{
	fl t=sqrt(1/fl((v[0]*v[0])+(v[1]*v[1])+(v[2]*v[2])));
...
//PRAGMA_HLS(HLS array_partition variable=vmap factor=partition_factor BLOCK)
	...
	#endif
	{
		...
		for(dint32 i=0;i<(1+KNOB_WINDOW_SIZE_X)*3;i++)
			data0[i]=mem1.data[i];
		for(dint32 i=0;i<(KNOB_WINDOW_SIZE_X)*3;i++)
			data1[i]=mem1.data[(cols*3)+i];
loop1:for(dint32 i=0,j=0;i<(rows)*cols*3;i=i+(KNOB_WINDOW_SIZE_X*3))
		{
//		PRAGMA_HLS(HLS UNROLL FACTOR=outer_unroll)

			loop_2:for(dint32 x=0;x<KNOB_WINDOW_SIZE_X*3;x++,j++)
			{
//			PRAGMA_HLS(HLS UNROLL FACTOR=inner_unroll1)
				...
				if (!isnan (v00[0]) && !isnan (v01[0]) && !isnan (v10[0]))
				{
					...
					// normalized
					fl t=ac_sqrt_pwl <fl> (1/fl((r[0]*r[0])+(r[1]*r[1])+(r[2]*r[2])));
					for (dint32 i=0;i<3;i++)
						r[i]=r[i] * t;
...
loop_3:for(dint32 x = 0; x < KNOB_WINDOW_SIZE_X; x++)
			{
...
loop_4:for(dint32 x = 0; x < KNOB_WINDOW_SIZE_X; x++)
			{
		//	PRAGMA_HLS(HLS UNROLL FACTOR=inner_unroll2)
