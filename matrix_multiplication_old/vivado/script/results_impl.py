...
UNROLL_FACTOR1= [1,2,3,4,5,6,7,8]
UNROLL_FACTOR2=[1,2,3,4,5,6,7,8]

allCombinations = list(itertools.product(
    PARTITION_FACTOR,        # 0
    SUBDIM_X,       # 1
    UNROLL_FACTOR1,UNROLL_FACTOR2    ))
# ***************************************************************************
...
def main():
    ...
    file1.write("n"+","+"knob_PARTITION_FACTOR"+","+"knob_SUBDIM_X"+","+"knob_UNROLL_FACTOR1"+","+"knob_UNROLL_FACTOR2"+","+"obj1"+","+"obj2\n")
    for d in sorted(glob.glob('impl_reports/matrix_mul_x_export*.xml')):
        m = re.search('matrix_mul_x_export(\d+)', d)
        num = m.group(1)
