...
class Archive(object):
    ...
    def __init__(self, epsilons):
        ...
        self.secondary_solutions = [] # Objectives that are normally rejected (Quentin's addition)
    ...
    def add(self, objectives, tagalong, ebox):
        ...
        self.secondary_solutions.append([])

    def add_secondary(self, index, tagalong):
        """ add a secondary solution to the archive, plus auxiliary information """
        self.secondary_solutions[index].append(tagalong)

    def replace(self, index, objectives, tagalong, ebox):
        """ add a solution to the archive, plus auxiliary information """
        old_data = self.archive[index], self.tagalongs[index], self.boxes[index]
        self.archive[index] = objectives
        self.tagalongs[index] = tagalong
        self.boxes[index] = ebox
        return old_data
    ...
    def remove(self, index):
        ...
        self.secondary_solutions.pop(index)
    ...
    def sortinto(self, objectives, tagalong=None):
        ...
        while ai < asize - 1:
            ...
            if adist < sdist: # archive dominates
                self.add_secondary(ai, tagalong)
                return
            else: # solution dominates
                old = self.replace(ai, objectives, tagalong, ebox)
                self.add_secondary(ai, old[1])
                return
#                 self.remove(ai)
#                 ai -= 1
#                 asize -= 1
...
def eps_sort(tables, objectives=None, epsilons=None, **kwargs):
    ...
    *attribution*   True: add table number, row number to rows
    ...
    tagalongs = eps_sort_solutions(tables, epsilons, secondary)

    return tagalongs

def eps_sort_solutions(tables, epsilons=None, secondary=False):
    """
    ...
    if secondary:
        return archive.tagalongs + [item for sublist in archive.secondary_solutions for item in sublist]
    else:
