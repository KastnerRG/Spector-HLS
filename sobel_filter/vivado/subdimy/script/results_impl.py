...
def main():
    ...
    file1=open('final_result_impl_sobely.csv','w')
    file1.write("n"+","+"knob_DIMX_PARTITION_FACTOR"+","+"knob_DIMY_PARTITION_FACTOR"+","+"knob_UNROLL_FACTOR"+","+"knob_SUBDIM_Y"+","+"obj1"+","+"obj2\n")
    for d in sorted(glob.glob('impl_reports/sobel_y_export*.xml')):
        print(d)
        m = re.search('sobel_y_export(\d+)', d)
        num = m.group(1)
